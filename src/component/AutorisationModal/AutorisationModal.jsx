    ...
    return (
      <div className={cls.wrap}>
        ...
        <form onSubmit={handleSubmit}>
          <label>
            Логин:
            {(login.isDirty && login.emailError) && <div style={{ color: 'red' }}>Некорректная почта</div>}
            <input onChange={e => login.onChange(e)} onBlur={e => login.onBlur(e)} value={login.value} type="text" />
          </label>
          <label>
            Пароль:
            {(password.isDirty && password.minLengthError) && <div style={{ color: 'red' }}>Слишком короткий пароль</div>}
            <input onChange={e => password.onChange(e)} onBlur={e => password.onBlur(e)} value={password.value} type="text" />
          </label>
